...
export interface AnimatedGridPatternProps extends Omit<SVGProps<SVGSVGElement>, 'width' | 'height'> {
  width?: number
  height?: number
  x?: number
  y?: number
...
export function AnimatedGridPattern({
  ...
  function getPos() {
    if (!dimensions.width || !dimensions.height) return [0, 0] as [number, number]
    return [
  ...
  return (
    <svg
      ...
      aria-hidden="true"
      className={cn(
        'pointer-events-none absolute inset-0 h-full w-full fill-current stroke-current text-ink/[0.12]',
        className,
      )}
      {...props}
    >
